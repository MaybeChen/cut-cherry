...
脚本会在安装 SAM3 前后执行兼容版本对齐：`setuptools<81`、`numpy>=1.26,<2`、`opencv-python<4.13`、`scipy<1.18`、`tifffile<2026`，并安装 SAM3 本地 runtime 会 import 的 Triton 依赖（Linux/macOS 默认 `triton`，Windows Git Bash/MSYS/Cygwin 默认 `triton-windows`），最后运行 `pip check`。这组约束同时满足 `sam3 0.1.0` 的 `numpy<2,>=1.26` 与 `paddlex 3.7.1` 的 `numpy<2.4,>=1.24`，避免安装过程中被升级到 `numpy 2.5.0` 后产生依赖冲突；`setuptools<81` 用于避免 SAM3 源码中 `pkg_resources` 导入在新版 setuptools 下反复打印弃用警告。如果你的运行环境有更严格的制品库约束，可以用 `NUMPY_SPEC`、`TRITON_SPEC` 等变量覆盖：

```bash
SETUPTOOLS_SPEC="setuptools<81" NUMPY_SPEC="numpy>=1.26,<2" OPENCV_SPEC="opencv-python<4.13" TRITON_SPEC="triton-windows" bash scripts/setup_sam3.sh
```
