...
class PreprocessProcessor:
    def run(self, ctx: PipelineContext) -> None:
        if ctx.input_path.suffix.lower() == ".pdf":
            raise InputValidationError("PDF rendering is reserved for Phase 2; provide a page image in Phase 1")
        ...
        source = ctx.job_dir / "source.png"; normalized = ctx.job_dir / "normalized.png"
        shutil.copyfile(ctx.input_path, source); image.save(normalized)
        arr = cv2.cvtColor(np.array(image.convert("RGB")), cv2.COLOR_RGB2BGR)
        gray = cv2.cvtColor(arr, cv2.COLOR_BGR2GRAY)
        edges = cv2.Canny(gray, 50, 150)
        cv2.imwrite(str(ctx.job_dir / "gray.png"), gray); cv2.imwrite(str(ctx.job_dir / "edges.png"), edges)
        ...
        image.copy().thumbnail((512, 512)); image.convert("RGB").save(ctx.job_dir / "preview.png")
        ctx.artifacts.update({"source": source, "normalized": normalized, "gray": ctx.job_dir/"gray.png", "edges": ctx.job_dir/"edges.png"})
