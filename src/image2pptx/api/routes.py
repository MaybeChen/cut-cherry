...
def create_job(file: UploadFile = File(...), settings: Settings = Depends(get_settings)) -> JobResponse:
    suffix=Path(file.filename or "input.png").suffix.lower()
    if suffix not in {".png",".jpg",".jpeg",".webp",".pdf"}: raise HTTPException(400,"unsupported file type")
    tmp=Path(tempfile.mkdtemp())/("input"+suffix)
    with tmp.open("wb") as f: shutil.copyfileobj(file.file, f)
    try: result=ImageToPptxPipeline(settings).run(tmp)
    ...
    return JobResponse(job_id=result.job_id,status="completed",pptx_path=str(result.pptx_path),ir_path=str(result.ir_path))
@router.get("/jobs/{job_id}", response_model=JobResponse)
def get_job(job_id: str) -> JobResponse:
    rec=STORE.get(job_id)
    if not rec: raise HTTPException(404,"job not found")
    return JobResponse(job_id=rec.job_id,status=rec.status,pptx_path=rec.pptx_path,ir_path=rec.ir_path)
...
def download(job_id: str):
    rec=STORE.get(job_id)
    if not rec or not rec.pptx_path: raise HTTPException(404,"pptx not found")
